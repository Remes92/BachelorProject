...
public static class ProductFactory
{
    public static Product Create(MySqlDataReader row)
    {
        ...
        Product product = new Product()
        {
            ...
            Product_Price = Convert.ToDecimal(row[14]),
            Product_Interests = row[15].ToString(),
            Product_Category = row[16].ToString(),
            Product_CommissionPrice = Convert.ToDecimal(row[17].ToString()),
            Product_SupplierId = row[18].ToString(),
            Product_Rating = Convert.ToDecimal(row[19].ToString()),
            Product_Key = row[20].ToString(),
            Product_Max = row[21].ToString(),
            Product_Name = row[22].ToString(),
            Product_Price2 = Convert.ToDecimal(row[23].ToString()),
            Product_Parameters = row[24].ToString(),
            ...
            Product_Level = Convert.ToDecimal(row[29].ToString()),
            Product_ApprovedDate = (DateTime)row[30],
            ResellerId = row[31].ToString(),
            Product_Retention = (decimal)row[32],
            Product_Rule1 = Convert.ToDecimal(row[33].ToString()),
            Product_Rule2 = Convert.ToDecimal(row[34].ToString()),
            Product_Rule3 = Convert.ToDecimal(row[35].ToString()),
